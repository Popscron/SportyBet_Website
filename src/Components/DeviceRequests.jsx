...
const DeviceRequests = () => {
  ...
  const handleApproveWithDevice = () => {
    ...
    if (requestId) {
      // If devices are selected, send them; otherwise send empty array (backend will handle it)
      approveRequest(requestId, selectedDeviceIds && selectedDeviceIds.length > 0 ? selectedDeviceIds : null);
    }
  };

  const handleApproveWithoutDevice = () => {
    const requestId = selectedRequest?._id;
    if (requestId) {
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        {actionLoadingId === selectedRequest?._id ? (
                          ...
                          `Approve & Logout ${selectedDeviceIds.length} Device(s)`
                        ) : (
                          "Approve Without Logout"
                        )}
                      </button>
                    </div>
                    {selectedDeviceIds && selectedDeviceIds.length === 0 && (
                      <p className="text-xs text-yellow-400 text-center">
                        ⚠️ No devices selected. This will approve the request without logging out any devices.
                      </p>
                    )}
                  </div>
                </>
